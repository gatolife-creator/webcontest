...
export const Certificate = () => {
    ...
    const preload = (p5: P5) => {
        const img = p5.loadImage(process.env.PUBLIC_URL + "imgs/certificate-image.png");
        setImage(img);
    }
    ...
    const draw = (p5: P5) => {
        ...
        p5.textSize(20);
        p5.text("ニックネームのハッシュ値は： " + SHA1(name), p5.width / 2, p5.height * 0.5 + 25);

        p5.textSize(30);
        p5.text(`${now.getFullYear()}年${now.getMonth() + 1}月${now.getDate()}日`, p5.width * 0.2, p5.height * 0.85 - 10)
    }

    return <>
        <Sketch className="certificate" preload={preload} setup={setup} draw={draw} />
        ...
        <div className="mx-auto">
            ...
            <a href={url} className="btn btn-success mt-5 text-right" download="認定証.png">
                <FaDownload />
            </a>
        </div>
    </>;
