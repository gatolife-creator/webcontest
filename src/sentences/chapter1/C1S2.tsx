  ...
  return (
    <Main duration="long">
      ...
      <Balloon char="node">
        ...
        {lang === "ja" && "ブロックチェーンの前身となったアイデアは、1991年にまでさかのぼる。"}
        {lang === "en" && "The idea was the forerunner of blockchain, dates back to 1991."}
      ...
      <Balloon char="node">
        {lang === "ja" && <>デジタル文書に<Important>タイムスタンプ</Important>
          をつけることで、文書の改ざんを防ぐというものだ。スチュアート・ヘイバー氏とスコット・ストルネッタ氏によって発表された。</>}
        {lang === "en" && <>The idea was to put <Important>a time stamp</Important> on digital documents to prevent them from being tampered with. It was presented by Stuart Haver and Scott Stornetta.</>}
      ...
      <Balloon char="node">
        ...
        {lang === "ja" && <> 簡単にいうと、データが<Important>作成された日時</Important>
          、それと<Important>改ざんされていない</Important>
          ことを証明する技術だ。</>}
        {lang === "en" && <>In brief, it is a technology that proves <Important>the date created</Important>, and <Important>no tampering</Important>.</>}
      ...
      <Balloon char="node">
        ...
        {lang === "ja" && "あぁ、しかし残念なことに、ビットコインが誕生する4年まえに特許が切れてしまったのだ。"}
        {lang === "en" && "Ah, but, unfortunately the patent expired four years before Bitcoin was born.Reference:'How to Time-Stamp a Digital Document.'"}
      ...
      <Image
        ...
        caption={(lang === "ja" ? "引用元" : lang === "en" ? "Reference Source" : "") + ": Bitcoin Forum"}
        style={{}}
      ></Image>
      <Balloon char="node">

        {lang === "ja" && <> 2008年、サトシ・ナカモトを語る人物が、
          <Important>ビットコイン</Important>
          （Bitcoin） と呼ばれる電子決済システムについての論文を発表した。</>}

        {lang === "en" && <>In 2008, a person called Satoshi Nakamoto published a paper on an electronic payment system <Important>“Bitcoin”</Important>.</>}

      </Balloon>

      <Balloon char="node">

        {lang === "ja" && "そして、2009年に運用開始。遂に世界初の仮想通貨、ビットコインの誕生だ！"}
        {lang === "en" && "And it became operational in 2009. At last, the world's first cryptol currency, bitcoin was born!"}
      </Balloon>
