...
先生：うむ。いい質問だ。実は、**改ざん検知**や**認証システム**に活用されているのだよ。今回は、**改ざん検知**に焦点を当てて解説するぞ。
...
先生：何も対策をしていなければ、データが改ざんされても、それを知る術はないのだ。

生徒：もしも機密情報だったりしたら、大変ですね。
...
先生：公開鍵で復号化できるようにすることで、誰もが復号化できるようにしている。

生徒：それじゃあ暗号化の意味ないじゃないですか！

先生：そこが重要なのだよ。誰もが復号化できるようにすることで、誰もが署名（データ送信者の照合）を確認することができるのだ。

***
※否認・・・データを送信したにも関わらず、送信したことを否定すること。インターネット上において、どれが元のデータであるか区別することは難しいため、デジタル署名のような方法が採用されている。
***
